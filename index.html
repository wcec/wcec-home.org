---
layout: home
title: 威明顿主恩堂
published: true
---
<table>
<tr><td>
<h1> 2017 年度主题: 凡事长进,连于元首基督</h1>
</td></tr>
<tr><td>
<div style ="align:cnter;">
<!--
	<marquee 
     direction="left"
     loop="7"
     scrollamount="2"
     scrolldelay="1"
     behavior="scroll"
     width="75%"
     bgcolor="#ff3424"
     style="color:white;font-weight:bold;"
  >
歡迎參加感恩节爱宴和特別聚會&nbsp;(11/24/2017),&nbsp;来感谢神一年来的保守。	
</marquee>
-->
<!--
<a href="http://wcec-home.org/event/2017-thanksgiving-events" target="_blank" >
<h3 style="color:white; background:red;font-weight:bold;padding: 15px;">
歡迎參加這個星期五感恩节爱宴和特別聚會(11/24/2017)</h3>	
</a>
-->
</div>
</td></tr>
<tr><td>
<div style ="align:cnter;">
<h3 style="color:white; background:red;font-weight:bold;padding: 15px;">
歡迎參加主日崇拜 (11:00 AM, 12/3/2017)&nbsp;.信息主題:寒夜里的爱与光<br><br>
歡迎參加受洗入會典礼 (英文部-9:30 AM, 12/3/2017. 中文部-1:30 AM, 12/3/2017)&nbsp;.</h3>	
</div>
</td></tr>
</table>
<table>
<!-- 
  <tr>
    <td><h2>冬季主日学 -- 欢迎参加</h2></td>
  </tr> 
  <tr>
    <td>
      <ul>
        <li>诗歌智慧文学 (诗篇+箴言书)</li> 
        <li>诗的故事与作者～美国圣诗（二）</li>
        <li>认识耶稣 (新约)</li>
        <li>信徒造就班</li>
        <li>福音真理班</li> 
      </ul>
    </td> 
  </tr>
-->  
  <tr>
    <td><h2>最新活动</h2></td> 
  </tr>
  <tr>
    <td>
      <ul>
	<li>
	  <h3><a href="{{site.base}}/pages/2017AmazonSmile/" target="_blank">Amazon Smile</a> 
	</li>
      </ul>
    </td>
  </tr> 
</table>

<div class="row">
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="section-cell" onclick="top.location.href='/category/update/'"> 
      <img src="http://media.wcec-home.org/image/recent_update.jpg"/>
      <span class="label label-success">近期更新</span>
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="section-cell" > 
      {% include event.html %}
      <span class="label label-primary">活动通知 </span>
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="section-cell"
    onclick="top.location.href='/category/message/'"> 
      <img src="http://media.wcec-home.org/image/message_1.jpg" /> <br/>
      <span class="label label-default">主日信息 </span> 
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4">
    <div class="section-cell" onclick="top.location.href='/ganlin/'"> 
      <img src="http://media.wcec-home.org/doc/ganlin/ganlin_2015_1.png" class="section"/> <br/> 
      <span class="label label-info">甘霖期刊</span>
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4" id="clear1">
    <div class="section-cell"
    onclick="top.location.href='/category/children_ministry/'"> 
     <img src="http://media.wcec-home.org/image/children_1.jpg" class="section"/> <br/> 
      <span class="label label-warning">儿童事工 </span>
    </div>
  </div>
  <div class="col-xs-12 col-sm-6 col-md-4" id="clear2">
    <div class="section-cell" onclick="top.location.href='{% post_url /music/2015-03-01-choir-video %}'"> 
    <img src="http://media.wcec-home.org/image/piano_1.jpg" class="section"/> <br/> 
      <span class="label label-danger">教会献诗 </span>
    </div>
  </div>
</div>

<span class="clear"/>
